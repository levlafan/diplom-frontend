...
import CoAuthorModal from "../../components/CoAuthorModal";
...
export default function CreateComicPage() {
  ...
  return (
    <main className={styles.page}>
      <div className={styles.container}>
        <h1 className={styles.title}>Создать комикс</h1>
  ...
  <input 
    ...
    onChange={async (e) => { 
      ...
      if (!file) { setCoverImage(null); return; } 
      setCoverImage(await compressToWebp(file, "cover")); 
    }} 
  />
  <span className={styles.fileButton}>Выбрать файл</span>
  {coverPreview ? (
    <Image src={coverPreview} alt="Превью" className={styles.preview} width={120} height={120} unoptimized />
  ) : (
    <p>Файл не выбран</p>
  )}
</label>
...
<label className={styles.uploadBox} onDragOver={(e) => e.preventDefault()} onDrop={async (e) => { e.preventDefault(); await handleZipFile(e.dataTransfer.files?.[0]); }}>
  ...
  <span className={styles.fileButton}>Выбрать ZIP</span>
